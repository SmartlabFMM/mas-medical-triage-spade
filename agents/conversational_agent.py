...
Behaviours:
  - WatchPatientsBehaviour: polls Database for new patients every 10s
...
class ConversationalAgent(Agent):
    ...
    class ListenDecisionBehaviour(CyclicBehaviour):
        async def run(self):
            ...
            if msg_type == MessageType.FINAL_DECISION:
                ...
                # Write decision to Database so frontend can see it
                if self.agent.db and payload:
                    try:
                        ...
                        print(f"[DEBUG ListenDecision] Decision written to Database with status {final_status}!")
    ...
    # Behaviour 3 : Watch Database for new patients (unchanged)
    class WatchPatientsBehaviour(CyclicBehaviour):
        ...
        async def run(self):
            try:
                ...
                # Note: Database uses "statut" (French) not "status" (English)
                        ...
                        if isinstance(raw_symptoms, str):
                            ...
                            for s in raw_symptoms.split(","):
                                ...
                                if token:
                                    ...
                                    # Preserve Unicode characters in symptom tokens
# token = unicodedata.normalize("NFKD", token)  # keep diacritics
# No ASCII encoding to avoid loss of French characters
                        ...
                        # Récupérer symptoms_details depuis Database
    ...
    async def setup(self):
        ...
        try:
            self.db = SheetsDB()
            self.db.connect()
            log_agent_state("ConversationalAgent", "Database connected")
        except Exception as e:
            log_agent_state("ConversationalAgent",
        ...
        # Watch Database every 10 seconds
