  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (response.user.role === "patient" || response.user.role === "secretaire") navigate("/patient");
    ...
    >
      ...
      <div className="hidden lg:flex w-1/2 flex-col justify-between p-12 text-white">
        ...
        <div className="max-w-xl">
          ...
          >
            un système intelligent conçu pour les urgences médicales. Elle utilise une technologie avancée pour évaluer rapidement l'état des patients et organiser au mieux les ressources disponibles, assurant ainsi une prise en charge efficace et rapide.
          </motion.p>
        </div>
                ...
                >
                  ...
                  <div className="w-full md:w-1/3 flex flex-col items-center justify-center text-center gap-4 bg-[var(--bg-inset)] rounded-3xl p-8 border border-[var(--border)]">
                    <div className={`p-4 rounded-full ${role === 'medical' ? 'bg-[var(--danger-soft)] text-[var(--danger)]' : role === 'admin' ? 'bg-[var(--warning-soft)] text-[var(--warning)]' : 'bg-[var(--primary-soft)] text-[var(--primary)]'}`}>
                      {role === "patient" || role === "secretaire" ? <User className="w-8 h-8" /> : role === "medical" ? <Stethoscope className="w-8 h-8" /> : <Settings className="w-8 h-8" />}
                    </div>
                    <div>
                      <h3 className="font-semibold text-xl text-[var(--text-main)]">
                        {role === "patient" || role === "secretaire" ? "Espace agent d'accueil" : role === "medical" ? "Staff Médical" : "Agent Administratif"}
                      </h3>
                      {role === "secretaire" && (
